...
- **인증**: 비밀번호 3단계 (상위 등급은 하위 권한을 모두 포함)
  - `ADMIN_PASSWORD`(총관리자) — 매장 생성/삭제/이름변경, 백업·복원, 복구 스냅샷, 문의 답변, 변경 기록
  - `MANAGER_PASSWORD`(매장관리자) — 자기 매장 데이터는 모두 수정 가능(태그목록 포함). 문의함 사용 가능
  - `STAFF_PASSWORD`(사용자) — 조회 전용 + 개인 지정 태그(요청휴무)만 추가/삭제
